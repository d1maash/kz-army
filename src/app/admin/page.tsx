...
const AdminPage = () => {
    const [applications, setApplications] = useState<ApplicationsData>({ count: 0, results: [] });
    const pageSize = 20; // Should match your backend's default page size

    const fetchAllApplications = async (token: string): Promise<ApplicationsData> => {
        let allResults: Application[] = [];
        let currentPage = 1;
        let totalCount = 0;
